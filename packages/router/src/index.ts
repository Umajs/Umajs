...
export const Router = () => {
    ...
    for (const c of Uma.controllersInfo) {
        ...
        const methodNameArr: (string | number | symbol)[] = Reflect.ownKeys(clazz.prototype)
            .filter((name) => {
                return name !== 'constructor' && !decoratorMethodNameArr.includes(String(name)) && typeof clazz.prototype[`${String(name)}`] === 'function';
            });
        ...
        for (const m of methodMap.values()) {
            ...
            methodPath.forEach((p) => {
                ...
                if(!ALLROUTE.includes(String(routePath))){
                    ...
                    // 注册路由重复
                    console.error(`${routePath} ==> ${clazzName}.${methodName} has been registered. Recommended use the Path decorator to annotate the ${clazzName}.controller.ts`);
                    return;
                }
